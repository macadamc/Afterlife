...
{
    [MenuItem("Tools/ShadyPixel/Rebuild Tags And Layers Enums")]
    static void RebuildTagsAndLayersEnums()
    {
        var enumsPath = Application.dataPath + "/";

        rebuildTagsFile(enumsPath + "Tags.cs");
        rebuildLayersFile(enumsPath + "Layers.cs");

        AssetDatabase.ImportAsset("Tags.cs", ImportAssetOptions.ForceUpdate);
        AssetDatabase.ImportAsset("Layers.cs", ImportAssetOptions.ForceUpdate);
    }
