...
#if UNITY_EDITOR
        [MenuItem("Assets/Create/Rule Override Tile")]
        public static void CreatePipelineTile()
        {
            string path = EditorUtility.SaveFilePanelInProject("Save Rule Override Tile", "New Rule Override Tile", "asset", "Save Rule Override Tile", "Assets");

            if (path == "")
                return;

            AssetDatabase.CreateAsset(ScriptableObject.CreateInstance<RuleOverrideTile>(), path);
        }
#endif
    }
