...
{
	[CustomEditor(typeof(RuleTileExtended))]
	[CanEditMultipleObjects]
	internal class RuleTileExtendedEditor : Editor
	{
		...
		public RuleTileExtended tile { get { return (target as RuleTileExtended); } }
		...
		public void OnEnable()
		{
			if (tile.m_TilingRules == null)
				tile.m_TilingRules = new List<RuleTileExtended.TilingRule>();

			m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(RuleTileExtended.TilingRule), true, true, true, true);
		...
		{
            RuleTileExtended.TilingRule rule = tile.m_TilingRules[index];
		...
		{
            RuleTileExtended.TilingRule rule = new RuleTileExtended.TilingRule();
			rule.m_Output = RuleTileExtended.TilingRule.OutputSprite.Single;
		...
		internal static void RuleMatrixOnGUI(Rect rect, RuleTileExtended.TilingRule tilingRule)
		{
			...
			for (int y = 0; y <= 2; y++)
			{
				for (int x = 0; x <= 2; x++)
				{
					...
					if (x != 1 || y != 1)
					{
						...
						{
							case RuleTileExtended.TilingRule.Neighbor.This:
								GUI.DrawTexture(r, arrows[y*3 + x]);
								break;
							case RuleTileExtended.TilingRule.Neighbor.NotThis:
						...
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
						    ...
						    if (Event.current.button == 1)
								change = -1;
							tilingRule.m_Neighbors[index] = (RuleTileExtended.TilingRule.Neighbor) (((int)tilingRule.m_Neighbors[index] + change) % 3);
							GUI.changed = true;
							Event.current.Use();
						}
					...
					else
					{
						...
						{
							case RuleTileExtended.TilingRule.Transform.Rotated:
								GUI.DrawTexture(r, autoTransforms[0]);
								break;
							case RuleTileExtended.TilingRule.Transform.MirrorX:
								GUI.DrawTexture(r, autoTransforms[1]);
								break;
							case RuleTileExtended.TilingRule.Transform.MirrorY:
						...
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_RuleTransform = (RuleTileExtended.TilingRule.Transform)(((int)tilingRule.m_RuleTransform + 1) % 4);
							GUI.changed = true;
							Event.current.Use();
						}
					}
		...
		{
			public RuleTileExtended.TilingRule m_Rule;
			public RuleTileExtended.TilingRule.Transform m_NewValue;

			public MenuItemData(RuleTileExtended.TilingRule mRule, RuleTileExtended.TilingRule.Transform mNewValue)
			{
				this.m_Rule = mRule;
		...
		internal static void SpriteOnGUI(Rect rect, RuleTileExtended.TilingRule tilingRule)
		{
		...
		internal static void RuleInspectorOnGUI(Rect rect, RuleTileExtended.TilingRule tilingRule)
		{
			float y = rect.yMin;
			EditorGUI.BeginChangeCheck();
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
			tilingRule.m_RuleTransform = (RuleTileExtended.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RuleTransform);
			...
			tilingRule.m_Output = (RuleTileExtended.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
			y += k_SingleLineHeight;

			if (tilingRule.m_Output == RuleTileExtended.TilingRule.OutputSprite.Animation)
			{
			...
			if (tilingRule.m_Output == RuleTileExtended.TilingRule.OutputSprite.Random)
			{
				...
				tilingRule.m_RandomTransform = (RuleTileExtended.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
				y += k_SingleLineHeight;
			}

			if (tilingRule.m_Output != RuleTileExtended.TilingRule.OutputSprite.Single)
			{
		...
		{
			[SerializeField]
			public List<RuleTileExtended.TilingRule> rules = new List<RuleTileExtended.TilingRule>();
		}
		
		[MenuItem("CONTEXT/RuleTileExtended/Copy All Rules")]
		private static void CopyAllRules(MenuCommand item)
		{
			RuleTileExtended tile = item.context as RuleTileExtended;
			if (tile == null)
				return;
		...
		[MenuItem("CONTEXT/RuleTileExtended/Paste Rules")]
