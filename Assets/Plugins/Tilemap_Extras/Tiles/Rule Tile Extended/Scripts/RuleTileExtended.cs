...
namespace UnityEngine
{
	[Serializable]
	[CreateAssetMenu]
	public class RuleTileExtended : TileBase
	{
		...
		public bool RuleMatches(TilingRule rule, ref TileBase[] neighboringTiles, int angle)
		{
			for (int i = 0; i < NeighborCount; ++i)
			{
				...
				if (rule.m_Neighbors[i] == TilingRule.Neighbor.This && tile == null || rule.m_Neighbors[i] == TilingRule.Neighbor.NotThis && tile != null)
				{
					return false;
				}
			}
