...
namespace Sprache
{
    ...
    public static partial class Parse
    {
        ...
        public static Parser<object> Not<T>(this Parser<T> parser)
        {
            ...
            return i =>
            {
                ...
                if (result.WasSuccessful)
                {
                    ...
                    var msg = string.Format("`{0}' was not expected", StringExtensions.Join(", ", result.Expectations));
        ...
        public static Parser<IEnumerable<T>> XMany<T>(this Parser<T> parser)
        {
            ...
            return parser.Many().Then((IEnumerable<T> m) => parser.Once().XOr(Return(m)));
        }
        ...
        public static Parser<IEnumerable<T>> AtLeastOnce<T>(this Parser<T> parser)
        {
            ...
            return parser.Once().Then((IEnumerable<T> t1) => parser.Many().Select(ts => t1.Concat(ts)));
        }
        ...
        public static Parser<IEnumerable<T>> XAtLeastOnce<T>(this Parser<T> parser)
        {
            ...
            return parser.Once().Then((IEnumerable<T> t1) => parser.XMany().Select(ts => t1.Concat(ts)));
        }
        ...
        public static Parser<U> Select<T, U>(this Parser<T> parser, Func<T, U> convert)
        {
            ...
            return parser.Then((T t) => Return(convert(t)));
        }
        ...
        public static Parser<IEnumerable<T>> Concat<T>(this Parser<IEnumerable<T>> first, Parser<IEnumerable<T>> second)
        {
            ...
            return first.Then((IEnumerable<T> f) => second.Select(x => f.Concat(x)));
        }
        ...
        {
            return parser.Except(until).Many().Then((IEnumerable<T> r) => until.Return(r));
        }
            ...
            Func<T, U, V> projector)
        {
            ...
            return parser.Then((T t) => selector(t).Select((U u) => projector(t, u)));
        }
            ...
            Func<TOp, T, T, T> apply)
        {
            ...
            return operand.Then((T first) => ChainOperatorRest<T, TOp>(first, op, operand, apply, Or));
        }
            ...
            Func<TOp, T, T, T> apply)
        {
            ...
            if (operand == null) throw new ArgumentNullException("operand");
            if (apply == null) throw new ArgumentNullException("apply");
            return operand.Then((T first ) => ChainOperatorRest<T, TOp>(first, op, operand, apply, XOr));
        }
            ...
            Func<Parser<T>, Parser<T>, Parser<T>> or)
        {
            ...
            if (operand == null) throw new ArgumentNullException("operand");
            if (apply == null) throw new ArgumentNullException("apply");
            return or(op.Then((TOp opvalue ) =>
                          operand.Then((T operandValue ) =>
            ...
            Func<TOp, T, T, T> apply)
        {
            ...
            if (operand == null) throw new ArgumentNullException("operand");
            if (apply == null) throw new ArgumentNullException("apply");
            return operand.Then((T first ) => ChainRightOperatorRest<T, TOp>(first, op, operand, apply, Or));
        }
            ...
            Func<TOp, T, T, T> apply)
        {
            ...
            if (operand == null) throw new ArgumentNullException("operand");
            if (apply == null) throw new ArgumentNullException("apply");
            return operand.Then((T first ) => ChainRightOperatorRest<T, TOp>(first, op, operand, apply, XOr));
        }
            ...
            Func<Parser<T>, Parser<T>, Parser<T>> or)
        {
            ...
            return or(op.Then((TOp opvalue ) =>
                        operand.Then((T operandValue ) =>
                            ChainRightOperatorRest<T, TOp>(operandValue, op, operand, apply, or)).Then((T r ) =>
        ...
        {
            return Number.Then((string n ) => DecimalWithoutLeadingDigits(ci).XOr(Return("")).Select(f => n + f));
        }
